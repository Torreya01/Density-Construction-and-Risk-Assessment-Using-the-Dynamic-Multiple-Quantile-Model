...
kurtosis_list_apple = kde_apple_all %>%
  ...
  summarise(Kurtosis = kurtosis(Quantile, na.rm = TRUE)) %>%
  ungroup() 
...
# Apple
# Calculate the krutosis for each month in 2014 to 2024
variance_list_apple = kde_apple_all %>%
  group_by(Year, Month_num) %>%
  summarise(variance = var(Quantile, na.rm = TRUE)) %>%
  ungroup() 

# Create a new column 'Date' for plotting
variance_list_apple$Date = with(variance_list_apple, as.Date(paste(Year, Month_num, "1", sep = "-")))
variance_list_apple = variance_list_apple[order(variance_list_apple$Date), ]

# Plotting the kurtosis
ggplot(variance_list_apple, aes(x = Date, y = variance)) +
  geom_line() +  # Connect points with lines
  geom_point() +  # Show points
  labs(title = "Monthly variance over Time for Apple Inc.",
       x = "Date (Year)",
       y = "variance") +
  theme_minimal() + 
...
# FTSE
# Calculate the krutosis for each month in 2014 to 2024
variance_list_FTSE = kde_FTSE_all %>%
  group_by(Year, Month_num) %>%
  summarise(variance = var(Quantile, na.rm = TRUE)) %>%
  ungroup() 

# Create a new column 'Date' for plotting
variance_list_FTSE$Date = with(variance_list_FTSE, as.Date(paste(Year, Month_num, "1", sep = "-")))
variance_list_FTSE = variance_list_FTSE[order(variance_list_FTSE$Date), ]

# Plotting the kurtosis
ggplot(variance_list_FTSE, aes(x = Date, y = variance)) +
  geom_line() +  # Connect points with lines
  geom_point() +  # Show points
  labs(title = "Monthly variance over Time for FTSE 100 Index",
       x = "Date (Year)",
       y = "variance") +
  theme_minimal() + 
...
months_years = list(c(2018, 10), c(2023, 10), c(2018, 11), c(2022, 3), c(2018, 4), c(2021, 5))
